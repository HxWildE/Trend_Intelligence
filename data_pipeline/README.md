...
3. **Loading (`loaders/`)**: Reads the output CSVs and ingests them into PostgreSQL for historical tracking and querying.
...
- **What it does**: Holds base directory references, database credentials, specific URLs, Reddit keywords, high-value tech/news/finance subreddits to monitor, and scheduling delay intervals.

### 2. `collectors/`
Responsible for the "Extract" phase.
- **`reddit_collector.py`**: 
  - Iterates over predefined subreddits and custom dynamically generated keywords.
  - Queries the Reddit JSON API.
  - Automatically filters out low-value "megathreads" or "daily discussions" to keep downstream NLP topics accurate and concrete.
...
- **`db_loader.py`**:
  - Exposes `DataLoader` to handle database connection logic using `sqlalchemy`.
